...
async function getCfg() {
  const cfg = await ProviderConfig.findOne({ key: 'neco' }).lean();
  return {
    enabled: cfg?.enabled ?? false,
    sandbox: cfg?.sandbox ?? true,
    baseUrl: cfg?.baseUrl || process.env.NECO_API_BASE || '',
    apiKey:  cfg?.apiKey  || process.env.NECO_API_KEY  || '',
    cost:    cfg?.cost ?? 5,
  };
}
...
export async function verify(payload) {
  ...
  if (!cfg.baseUrl || !cfg.apiKey) throw new Error('NECO integration not configured');
  ...
  const data = await postJSON(`${cfg.baseUrl}/neco/verify`, body, {
